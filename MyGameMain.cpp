...
namespace  Ending {//ゲームエンディング
	extern  void  Initialize();
	extern  TaskFlag  UpDate();
	extern  void  Render();
	extern  void  Finalize();
}
namespace  NewGame {//ニューゲーム
	extern  void  Initialize();
	extern  TaskFlag  UpDate();
	extern  void  Render();
	extern  void  Finalize();
}
namespace  LoadGame {//ロードゲーム
	extern  void  Initialize();
	extern  TaskFlag  UpDate();
	extern  void  Render();
	extern  void  Finalize();
...
void  MyGameMain_Finalize()
{
	...
	switch (actTask) {
		...
		case TaskFlag::LoadGame:LoadGame::Finalize();break;
		//以下必要に応じて追加
	}
...
void  MyGameMain_UpDate()
{
	...
	if (actTask != nextTask) {
		...
		switch (actTask) {
			...
			case TaskFlag::LoadGame:	LoadGame::Initialize(); break;
			//以下必要に応じて追加
		}
	}
	//現在のタスクの実行・更新処理を呼びだす
	switch (actTask) {
		...
		case TaskFlag::LoadGame:	nextTask = LoadGame::UpDate();  break;
		//以下必要に応じて追加
	}
}
//-----------------------------------------------------------------------------
...
void  MyGameMain_Render2D()
{
	switch (actTask) {
		...
		case TaskFlag::LoadGame:	LoadGame::Render(); break;
		//以下必要に応じて追加
	}
}
